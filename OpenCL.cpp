...
TEST_F(OpenCL, AMFContext_GetOpenCLComputeFactory) {
	AMFComputeFactoryPtr fact;
	res = context1->GetOpenCLComputeFactory(&fact);
	EXPECT_NE(res, AMF_NOT_IMPLEMENTED);
}

TEST_F(OpenCL, AMFContext_InitDX11_GetDX11Device) {
	res = context1->InitDX11(context1->GetDX11Device());
	EXPECT_NE(res, AMF_NOT_IMPLEMENTED);
}

TEST_F(OpenCL, AMFContext_InitDX9_GetDX9Device) {
	res = context1->InitDX9(context1->GetDX9Device());
	EXPECT_NE(res, AMF_NOT_IMPLEMENTED);
}

TEST_F(OpenCL, AMFContext_InitOpenGL_GetOpenGLContext) {
	res = context1->InitOpenGL(context1->GetOpenGLContext(), NULL, NULL);
	EXPECT_NE(res, AMF_NOT_IMPLEMENTED);
}

TEST_F(OpenCL, AMFContext_InitXV_GetXVDevice) {
	res = context1->InitXV(context1->GetXVDevice());
	EXPECT_NE(res, AMF_NOT_IMPLEMENTED);
}

TEST_F(OpenCL, AMFContext_InitGralloc_GetGrallocDevice) {
	res = context1->InitGralloc(context1->GetGrallocDevice());
	EXPECT_NE(res, AMF_NOT_IMPLEMENTED);
}

TEST_F(OpenCL, AMFContext_LockOpenCL_UnlockOpenCL) {
